...
- `src/controls/dpad.ts` — moshi-style floating arrow-key pad (← ↑ ↓ → ⌫ ⏎ ⇥ ⇧⇥), toggled by the ✥ `dpad-toggle` action; keys come from `config.dpad.keys` (null = spacer cell), are focus-safe (touchend guard), and `send` keys go via `sendData` while other action types dispatch through the action registry
