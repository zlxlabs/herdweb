...
export default {
  ...
  drawer: {
    buttons: [
      { id: 'answer-1', label: '1', description: 'Answer option 1', action: { type: 'send', data: '1' }, section: 'Answer' },
      { id: 'answer-2', label: '2', description: 'Answer option 2', action: { type: 'send', data: '2' }, section: 'Answer' },
      ...
      { id: 'codex-transcript', label: 'Transcript', description: 'Codex: open transcript (Ctrl+T)', action: { type: 'send', data: '\x14' }, section: 'Codex' },
      { id: 'claude-mode', label: 'Mode', description: 'Claude Code: cycle mode (Shift+Tab)', action: { type: 'send', data: '\x1b[Z' }, section: 'Claude' },
      { id: 'claude-verbose', label: 'Verbose', description: 'Claude Code: verbose output (Ctrl+O)', action: { type: 'send', data: '\x0f' }, section: 'Claude' },
      { id: 'claude-tasks', label: 'Tasks', description: 'Claude Code: toggle tasks (Ctrl+T)', action: { type: 'send', data: '\x14' }, section: 'Claude' },
      { id: 'claude-model', label: 'Model', description: 'Claude Code: switch model (Alt+P)', action: { type: 'send', data: '\x1bp' }, section: 'Claude' },
      { id: 'pi-queue', label: 'Queue', description: 'Pi: queue follow-up (Alt+Enter)', action: { type: 'send', data: '\x1b\r' }, section: 'Pi' },
      { id: 'pi-think', label: 'Think', description: 'Pi: cycle thinking level (Shift+Tab)', action: { type: 'send', data: '\x1b[Z' }, section: 'Pi' },
      { id: 'pi-model', label: 'Model', description: 'Pi: cycle models (Ctrl+P)', action: { type: 'send', data: '\x10' }, section: 'Pi' },
      { id: 'pi-expand', label: 'Expand', description: 'Pi: toggle tool output (Ctrl+O)', action: { type: 'send', data: '\x0f' }, section: 'Pi' },
      { id: 'herdr-new-window', label: '+ Win', description: 'Create herdr window', action: { type: 'send', data: '\x02c' }, section: 'herdr' },
      { id: 'herdr-rename-tab', label: 'Rename', description: 'Rename current herdr tab', action: { type: 'send', data: '\x02T' }, section: 'herdr' },
      { id: 'herdr-split-v', label: 'Split |', description: 'Split pane vertically', action: { type: 'send', data: '\x02v' }, section: 'herdr' },
      { id: 'herdr-split-h', label: 'Split —', description: 'Split pane horizontally', action: { type: 'send', data: '\x02-' }, section: 'herdr' },
      { id: 'herdr-zoom', label: 'Zoom', description: 'Toggle pane zoom', action: { type: 'send', data: '\x02z' }, section: 'herdr' },
      { id: 'herdr-workspaces', label: 'Spaces', description: 'Choose herdr workspace', action: { type: 'send', data: '\x02w' }, section: 'herdr' },
      { id: 'herdr-sidebar', label: 'Sidebar', description: 'Toggle herdr sidebar', action: { type: 'send', data: '\x02b' }, section: 'herdr' },
      { id: 'herdr-scrollback', label: 'Scroll', description: 'Enter herdr scrollback editor', action: { type: 'send', data: '\x02e' }, section: 'herdr' },
      { id: 'herdr-help', label: 'Keys', description: 'List herdr key bindings', action: { type: 'send', data: '\x02?' }, section: 'herdr' },
      { id: 'herdr-kill-pane', label: 'Kill', description: 'Kill current pane (with confirm)', action: { type: 'send', data: '\x02x' }, section: 'herdr' },
      { id: 'prefix', label: 'Prefix', description: 'Send herdr prefix key (Ctrl-B)', action: { type: 'prefix', data: '\x02' }, section: 'herdr' },
      { id: 'page-up', label: 'PgUp', description: 'Send Page Up key', action: { type: 'send', data: '\x1b[5~', keyLabel: 'Page Up' }, section: 'Terminal' },
      { id: 'page-down', label: 'PgDn', description: 'Send Page Down key', action: { type: 'send', data: '\x1b[6~', keyLabel: 'Page Down' }, section: 'Terminal' },
      { id: 'ctrl-c', label: 'C-c', description: 'Send Ctrl-C interrupt', action: { type: 'send', data: '\x03' }, section: 'Terminal' },
      { id: 'ctrl-d', label: 'C-d', description: 'Send Ctrl-D key', action: { type: 'send', data: '\x04' }, section: 'Terminal' },
      { id: 'q', label: 'q', description: 'Send q key', action: { type: 'send', data: 'q' }, section: 'Terminal' },
      { id: 'alt-enter', label: 'M-↵', description: 'Send Alt+Enter (ESC + Enter)', action: { type: 'send', data: '\x1b\r', keyLabel: 'Alt+Enter' }, section: 'Terminal' },
      { id: 'space', label: 'Space', description: 'Send Space key', action: { type: 'send', data: ' ' }, section: 'Terminal' },
      { id: 'ctrl', label: 'Ctrl', description: 'Sticky Ctrl modifier (applies to the next key)', action: { type: 'ctrl-modifier' }, section: 'Terminal' },
      { id: 'combo-picker', label: 'Combo', description: 'Open combo sender (Ctrl/Alt + key)', action: { type: 'combo-picker' }, section: 'Terminal' },
      { id: 'paste', label: 'Paste', description: 'Paste from clipboard', action: { type: 'paste' }, section: 'Terminal' },
      { id: 'font-decrease', label: 'Font −', description: 'Decrease font size', action: { type: 'font-size', delta: -2 }, section: 'App' },
      { id: 'font-increase', label: 'Font +', description: 'Increase font size', action: { type: 'font-size', delta: 2 }, section: 'App' },
      { id: 'notify-panel', label: '🔔', description: 'Notification settings', action: { type: 'notify-panel' }, section: 'App' },
      { id: 'guide', label: 'Guide', description: 'Open the herdweb help guide', action: { type: 'help' }, section: 'App' },
      { id: 'select-mode', label: 'Select', description: 'Select and copy frozen terminal text', action: { type: 'select-mode' }, section: 'App' },
    ],
  },
  gestures: {
    swipe: {
...
via `$(date +%s000)` (Unix seconds with three trailing zeros). Ingress
range-checks `ts` and returns HTTP 400 when the value is below
